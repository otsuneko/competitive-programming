...
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/ahc030/awGA15Om_windows.zip).
...
- `--eps=0.1` Fix the $eps$ value of the generated inputs to the specified one.
...
You can visualize the contents of the output file by pasting it into the [visualizer](https://img.atcoder.jp/ahc030/awGA15Om.html?lang=en).
...
You can also use a [web visualizer](https://img.atcoder.jp/ahc030/awGA15Om.html?lang=en) which is more rich in features.
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc030/awGA15Om_windows.zip)も用意してあります。
...
- `--eps=0.1` 生成される入力の $eps$ の値を指定されたものに固定
...
出力された`out.txt`の中身を[ビジュアライザ](https://img.atcoder.jp/ahc030/awGA15Om.html?lang=ja)に貼り付けると、ビジュアライズが可能です。
...
より機能が豊富な[ウェブ版のビジュアライザ](https://img.atcoder.jp/ahc030/awGA15Om.html?lang=ja)も利用可能です。

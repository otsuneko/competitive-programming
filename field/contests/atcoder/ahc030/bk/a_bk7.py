...
TRIAL_DIG_CAND_NUM = 5 # 毎回の試し掘り場所決めの候補とする座標の数
DIST_WEIGHT = 3 # 試し掘りの候補マスの隣接マスにv(y,x)>0のマスがある場合の重み
DFS_LIMIT = 10**6
...
class Solver:
    ...
    def solve(self) -> int:
        ...
        while 1:
            ...
            # 2-1. Mの数が大きくない場合はDFSで最適な配置を探索する
            if self.M < 15:
                # 2-1-1. 確定情報を増やすために試し掘りする
                self.trial_dig()

                # 2-1-2. 現時点の情報を基に、最適な配置を探索し回答する
                oil_pos_set = self.find_optimal_arrangement()
                if oil_pos_set:
                    finish_flg = self.judge.answer(oil_pos_set)
                    if finish_flg:
                        return
            # 2-2. Mの数が大きい場合は全て掘り尽くして回答する
            else:
                # 2-2-1. 油田がある可能性があるグループ内の1マスを試し掘り。見つかったら隣接マス発掘の始点候補に追加
                v_S,y,x = self.dig_probable_pos()
                # 上限ターンに達するか全ての埋蔵石油が発掘できたら終了
                if self.judge_if_turn_over() or self.judge_if_digged_all():
                    break

                # 2-2-2. 全ての油田が見つかるまで、発掘済みの油田マスの隣接マスを掘る
                    ...
                    for dy in range(FORECAST_LEN):
                        for dx in range(FORECAST_LEN):
                            ...
                            if not (0<=y<self.N and 0<=x<self.N):
                                continue
                            pos_list.append((y,x))
                            if diff > 0 and self.judge.oil_forecast_map[y][x] != -1:
                                estimated_v_S += self.judge.oil_forecast_map[y][x]
                    # 占うためには2マス以上必要
                    if len(pos_list) > 1:
                        # 一度測定していて何もないマスの可能性が高い場合は再測定しない
                        if diff > 0 and estimated_v_S / len(pos_list) <= 0.1:
                            continue
    ...
    def trial_dig(self) -> None:
        ...
        """
        #TODO: ポリオミノの配置を確定させるには、外枠の0を見つけるように掘り先を決める。隣に1があるマスを優先とか。
        trial_dig_cnt = 0
        cur_y,cur_x = -1,-1
        while trial_dig_cnt < TRIAL_DIG_NUM and not (self.judge_if_turn_over() or self.judge_if_digged_all()):
            ...
            # 以降は遠くのマスを掘る
                ...
                for i,(v_S,(y,x)) in enumerate(self.trial_dig_cand_pos_list):
                    ...
                    for dy,dx in MOVE:
                        ny,nx = y+dy,x+dx
                        if 0<=ny<self.N and 0<=nx<self.N and self.judge.oil_map[ny][nx] > 0:
                            dist *= DIST_WEIGHT
                            break
                    if dist > max_dist:
                        max_dist = dist
    ...
    def find_optimal_arrangement(self):
        ...
        # print(self.judge.turn, file=sys.stderr)
        # print(*probable_poly_list, sep="\n",file=sys.stderr)
        # print(file=sys.stderr)

        pattern_num = 1
        for poly_id in range(self.M):
            if poly_id in self.judge.definitive_poly_id:
                continue
            pattern_num *= len(probable_poly_list[poly_id])
        # print(self.judge.turn, file=sys.stderr)
        # print(pattern_num,file=sys.stderr)
        # print(file=sys.stderr)
        if pattern_num > DFS_LIMIT:
            return set()

        # 確定済のポリオミノの配置は予め格納しておき、未確定ポリオミノの配置を探索
        estimated_poly_pos_map = [[set() for _ in range(self.N)] for _ in range(self.N)]
        definitive_pos_list = []
        for y in range(self.N):
            for x in range(self.N):
                if self.judge.definitive_poly_pos_map[y][x]:
                    estimated_poly_pos_map[y][x] |= self.judge.definitive_poly_pos_map[y][x]
                    definitive_pos_list.append((y,x))
        
        # print(self.judge.turn, file=sys.stderr)
        # print(*estimated_poly_pos_map, sep="\n",file=sys.stderr)
        # print(file=sys.stderr)

        # グリッドマップに書き込んでいき矛盾したらやり直す
        best_poly_pos_list = [] # 矛盾なく配置できたポリオミノの座標組合せを降順ソートしたリスト
        undefinitive_poly_id = list(set([i for i in range(self.M)]) - self.judge.definitive_poly_id)
        def search_best_poly_pos_dfs(i,total_v_S,estimated_poly_pos_map):
            # 最後まで矛盾なく配置できたらtotal_v_Sと配置を記録
            if i == len(undefinitive_poly_id):
                # print(self.judge.turn, file=sys.stderr)
                # print(*estimated_poly_pos_map, sep="\n", file=sys.stderr)
                # print(file=sys.stderr)
                if _judge_if_no_contradict(estimated_poly_pos_map):
                    final_pos_list = definitive_pos_list[:]
                    for y in range(self.N):
                        for x in range(self.N):
                            if estimated_poly_pos_map[y][x]:
                                final_pos_list.append((y,x))
                    final_pos_list.sort()
                    best_poly_pos_list.append((total_v_S,tuple(final_pos_list)))
                return

            # probable_poly_listの上位から順に組合せを試す
            poly_id = undefinitive_poly_id[i]
            oil = self.OILS[poly_id]
            for estimated_v_S,(base_y,base_x) in probable_poly_list[poly_id]:
                pos_list = []
                failure_flg = False
                for dy,dx in oil.relative_pos_list:
                    ny,nx = base_y+dy,base_x+dx
                    # 仮にポリオミノを置いた(+1)場合に、確定済v(y,x)の値を超える場合は矛盾
                    if self.judge.oil_map[ny][nx] != -1 and len(estimated_poly_pos_map[ny][nx]) + 1 > self.judge.oil_map[ny][nx]:
                        failure_flg = True
                        break
                ...
                for y,x in pos_list:
                    estimated_poly_pos_map[y][x].add(poly_id)
                search_best_poly_pos_dfs(i+1, total_v_S+estimated_v_S, estimated_poly_pos_map)
                # 後処理
                for y,x in pos_list:
                    estimated_poly_pos_map[y][x].remove(poly_id)
                    
        
        search_best_poly_pos_dfs(0,0,estimated_poly_pos_map)
        best_poly_pos_list.sort(reverse=True)

        # print(best_poly_pos_list, file=sys.stderr)
        
        # 回答用の座標セットを作成
        if best_poly_pos_list:
            poly_pos_set = set(best_poly_pos_list[0][1])
            if poly_pos_set:
                return poly_pos_set
        return set()

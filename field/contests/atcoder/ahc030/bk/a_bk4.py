...
class Solver:
    ...
    def forecast_optimal_poly_pos(self):
        ...
        for i,oil in enumerate(self.OILS):
            for j in range(len(self.probable_poly_list[i])):
                v_S,(base_y,base_x) = self.probable_poly_list[i][j]
                forecast_poly_pos = _calc_forecast_poly_pos(base_y, base_x, oil)
                ...
                if j > max(self.N, len(self.probable_poly_list[i])//4) and self.best_poly_pos_list[i][0][0] > len(forecast_poly_pos):
                    break
    ...
    def dig_adjacent_pos(self,v_S,y,x):
        ...
        while start_pos:
            ...
            if self.judge.oil_map[y][x] == -1:
                ...
                cnt += 1
                # 上限ターンに達するか全ての埋蔵石油が発掘できたら終了
                if self.judge_if_turn_over() or self.judge_if_digged_all() or cnt == 5:
                    return
            else:

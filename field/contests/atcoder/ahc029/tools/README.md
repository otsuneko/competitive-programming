...
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/ahc029/45e6da0b06_windows.zip).
...
- `--N=7` Fix the $N$ value of the generated inputs to the specified one.
- `--M=8` Fix the $M$ value of the generated inputs to the specified one.
- `--K=5` Fix the $K$ value of the generated inputs to the specified one.
...
You can also use a [web visualizer](https://img.atcoder.jp/ahc029/45e6da0b06.html?lang=en) which is more rich in features.
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc029/45e6da0b06_windows.zip)も用意してあります。
...
- `--N=7` 生成される入力の $N$ の値を指定されたものに固定
- `--M=8` 生成される入力の $M$ の値を指定されたものに固定
- `--K=5` 生成される入力の $K$ の値を指定されたものに固定
...
出力された`out.txt`の中身を[ビジュアライザ](https://img.atcoder.jp/ahc029/45e6da0b06.html?lang=ja)に貼り付けると、ビジュアライズが可能です。
...
より機能が豊富な[ウェブ版のビジュアライザ](https://img.atcoder.jp/ahc029/45e6da0b06.html?lang=ja)も利用可能です。

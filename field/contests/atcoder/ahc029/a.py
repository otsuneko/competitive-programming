...
class Solver:
    ...
    def _evaluate_projects(self):
        ...
        for idx, project in enumerate(self.projects):
            project_value = project.v - project.h
        ...
        match next_card.t:
            case CardType.WORK_SINGLE:
                if self.turn < 250:
                    if next_card.p < self.money / 2:
                        card_price = (next_card.w - next_card.p) * max(0, self.m//2-self.owned_card_num[next_card.t])
                    else:
                        card_price = (next_card.w - next_card.p) * max(0, self.m//2-self.owned_card_num[next_card.t])
                else:
                    card_price = (next_card.w - next_card.p) * max(0, self.m//2-self.owned_card_num[next_card.t])
            case CardType.WORK_ALL:
                if next_card.p < next_card.w * self.m * 0.75:
                    card_price = (next_card.w*self.m - next_card.p) * max(0, self._clamp(self.m,5,6)-self.owned_card_num[next_card.t])
                else:
                    card_price = (next_card.w*self.m - next_card.p) * max(0, self._clamp(self.m,2,3)-self.owned_card_num[next_card.t])

...
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/ahc016/d5f3c281_windows.zip).
...
- `--eps=0.01` Fix the $\epsilon$ value of the generated inputs to the specified one.
...
You can visualize the contents of the output file by pasting it into the [visualizer](https://img.atcoder.jp/ahc016/d5f3c281.html).
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc016/d5f3c281_windows.zip)も用意してあります。
...
- `--eps=0.01` 生成される入力の $\epsilon$ の値を指定されたものに固定
...
出力された`out.txt`の中身を[ビジュアライザ](https://img.atcoder.jp/ahc016/d5f3c281.html)に貼り付けると、ビジュアライズが可能です。

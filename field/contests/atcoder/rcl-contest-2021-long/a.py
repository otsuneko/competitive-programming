...
# 乱数シミュレーション用のパラメータ
SIMU_LOOP_FIRST = 0 # 序盤のシミュレーション回数(0なら乱数シミュレーションしない)
...
class Game:
    ...
    def find_contours(self, r, c):
        ...
        while next != start:
            for i in range(4):
                vec_new = (vec_old+i)%4
                next = [cur[0]+dir_dict[vec_new][0], cur[1]+dir_dict[vec_new][1]]
                if 0 <= next[0] < self.n and 0 <= next[1] < self.n and self.has_machine[next[0]][next[1]]:
    ...
    def search_machine_move(self, day, rand_flg):
        ...
        for machine in self.machines:
            r1,c1 = machine
            harvest = self.harvest_change_around[day][r1][c1]
            ...
            for c in range(self.n):
                ...
                if move_before != [-1,-1]:
                    ...
                    for dir in range(4):
                        ...
                        if 0 <= nr < self.n and 0 <= nc < self.n and self.has_machine[nr][nc]:
                            cnt_adjacent += 1
                    if self.num_machine > 2 and cnt_adjacent == 0:
                        continue
                    self.move(*move_before,r,c)
                    harvest = self.harvest_change_around[day][r][c]
        ...
        for r in range(self.n):
            for c in range(self.n):
                ...
                for dir in range(4):
                    ...
                    if 0 <= nr < self.n and 0 <= nc < self.n and self.has_machine[nr][nc]:
                        cnt_adjacent += 1
                if self.num_machine > 0 and cnt_adjacent == 0:
                    continue
                # 今後最も高い収穫量を見込める位置
                harvest = self.future_value_total_around[day][r][c]

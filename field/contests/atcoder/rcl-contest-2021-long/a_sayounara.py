...
PRUNE_NUM = 2 # 枝刈り後、終盤に回す候補数
SIMU_LOOP_LAST = 2 #終盤のシミュレーション
SWITCH_T_FIRST = 50 # 序盤の乱数シミュレーション開始ループ番号
SWITCH_T_MIDDLE = 200 # SWITCH_T_START～SWITCH_T_MIDDLEがSIMU_LOOP_FIRST回のシミュレーション対象
...
class Game:
    def __init__(self, n, t, veges):
        ...
        self.connected_machines = [[1] * self.n for _ in range(self.n)]
        self.next_price = 1
        self.num_machine = 0
        self.money = 1
    ...
    def simulate(self, day, action):
        ...
        if len(action.vs) == 2:
            ...
            self.move(*action.vs)
        # harvest
        # connected_machinesを全ての機械に対して前計算
        self.connected_machines = [[1] * self.n for _ in range(self.n)]
        for machine in self.machines:
            self.count_connected_machines(machine[0],machine[1],True)
        for r in range(self.n):
            for c in range(self.n):
                if self.has_machine[r][c] and self.harvest_change[day][r][c] > 0:
                    self.money += self.harvest_change[day][r][c] * self.connected_machines[r][c]
    ...
    def count_connected_machines(self, r, c, flag):
        if flag and self.connected_machines[r][c] != 1:
            return
        ...
        # 計算済みの連結成分の繰り返し計算削減用のメモ
        for q in queue:
            self.connected_machines[q[0]][q[1]] = i
    ...
    def search_machine_move(self, day, rand_flg):
        ...
        if self.num_machine == 1:
                ...
                move_after, _ = self.search_max_pos_move(day, rand_flg)
                ...
                for machine2 in self.machines:
                    ...
                    for dir in range(4):
                        ...
                        if 0 <= nr < self.n and 0 <= nc < self.n and not self.has_machine[nr][nc] and self.harvest_change[day][nr][nc] > 0:
                            ...
                            harvest = self.harvest_change_around[day][nr][nc] * self.count_connected_machines(nr,nc,False)
            ...
            for machine in self.machines:
                r1,c1 = machine
                if self.num_machine <= 5:
                    harvest = self.future_value_total_around[day][r1][c1] * self.connected_machines[r1][c1]
                else:
                    harvest = self.harvest_change[day][r1][c1] * self.connected_machines[r1][c1]
    ...
    def search_max_pos_move(self, day, rand_flg, move_before=[-1,-1]):
        # RAND_NUM=2の時限定の決め打ち実装
        pos_list = [[0, [-1,-1]], [0, [-1,-1]]]
        future = day
            ...
            for r in range(self.n):
                for c in range(self.n):
                    ...
                    if move_before != [-1,-1]:
                        ...
                        harvest = self.harvest_change_around[day][r][c] * self.count_connected_machines(r,c,False)
                        self.move(r,c,*move_before)
                    else:
                        harvest = self.harvest_change_around[future][r][c] * self.count_connected_machines(r,c,False)
                    if harvest > pos_list[0][0]:
                        pos_list[1] = [pos_list[0][0], [*pos_list[0][1]]]
                        pos_list[0] = [harvest, [r,c]]
                    elif harvest > pos_list[1][0]:
                        pos_list[1] = [harvest, [r,c]]
        ...
        if pos_list[0][0]:
            if rand_flg:
                pos = random.choices(pos_list,k=1,weights=WEIGHT)
        ...
        for r in range(self.n):
            for c in range(self.n):
                if self.has_machine[r][c]:
                    continue
                # 今後最も高い収穫量を見込める位置
                harvest = self.future_value_total_around[day][r][c] * self.count_connected_machines(r,c,False)
    ...
    def select_next_action(self, day, rand_flg=False):
        ...
        if self.money < self.next_price:
            ...
            # ループの終盤では機械を購入できなくする
            if purchase_pos != [-1,-1] and day < 865:

...
class Solver:
    ...
    def investigate_field_additional(self, start_house):
        queue = deque([start_house])
 
        while queue:
            cur = queue.popleft()
            # 水源の周囲まで辿り着いた場合は探索を終了
            for water in self.water_pos:
                if abs(cur.y - water.y) + abs(cur.x - water.x) <= INVESTIGATE_DIST*2:
                    # 発見済の水源リストに追加
                    self.found_water_pos.add(water)
                    # スタート地点の家から水源に至るルートを発見済として更新
                    self.is_house_connect_to_water[start_house] = True
            ...
            # 次の調査エリアを決める
            for dy,dx in MOVE:
                nxt = Pos(cur.y + dy*INVESTIGATE_DIST, cur.x + dx*INVESTIGATE_DIST)
                if (not (0<=nxt.y<self.N and 0<=nxt.x<self.N)):
                    continue
                # 調査済だが水源に至らない調査エリアの場合は掘削せずに次の調査エリアへ
                if self.field.is_investigated[nxt.y][nxt.x] == INVESTIGATED_STATUS.INVESTIGATED:
                    queue.append(nxt)
                    continue

                # 次に調査するエリアの周辺は水源に接続済とする
                self.update_investigated_area(nxt,INVESTIGATED_STATUS.CONNECT_TO_WATER)

                total_damage = 0
                # 追加調査では軟らかい岩盤のみを壊したいので、掘削回数を減らす
                for _ in range(self.INVESTIGATE_LOOP//8):
                    total_damage += self.INVESTIGATE_POWER
    ...
    def solve(self):
        ...
        # フェーズ1.5:水源に至る道がない時の追加調査
        # for house in self.is_house_connect_to_water:
        #     print(house.y,house.x,self.is_house_connect_to_water[house], file=sys.stderr)
        for house in self.is_house_connect_to_water:
            # 水源に至る道が未調査の場合
            if self.is_house_connect_to_water[house] == False:
                self.investigate_field_additional(house)

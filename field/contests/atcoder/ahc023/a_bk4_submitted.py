...
POS_SORT_BFS_DIST = 3 # 栽培マスを入り口から遠い順にソートする際に隣接マスをBFSで探索する時の距離
...
ALLOWED_D_DIFF = 1 # 通路上のマスに置ける最大のDからどれだけ引いた値まで許容するか
...
class Solver:
    ...
    def calc_longest_path(self):
        ...
        prev[self.y_exit][0] = [self.y_exit,0]
        visited = [[-1] * self.W for _ in range(self.H)]
        visited[self.y_exit][0] = 0
        longest_pos = [-1,-1]
        max_dist = -1

...
class Solver:
    ...
    # 初月だけの栽培計画の立て方
    def ini_cultivate_plan(self, sorted_cultivatable_pos,month,crops_order):

        # ゴールから入り口までの最短経路上の座標をスタートして、その両脇の道にひたすら
        # 同じ収穫時期の作物を栽培していく
        def greedy_cultivate(y,x,dy,dx):
            uncrop_pos_set.add((y,x))
            # 植える(ここに来た時点で植えられる座標であることは確定)
            for i in range(min(MAX_SEARCH_CROPS,len(crops))):
                crop_idx, s, d = crops[i]

                if [y,x] == [self.y_exit,0]:
                    break
                ...
                # 同じ収穫時期の作物がそれ以上植えられない場合は1ヶ月収穫時期が早い作物へ
                num_per_D[d] -= 1
                if num_per_D[d] == 0:
            ...
            # まずは同じ方向に突き進む
            ny,nx = y+dy,x+dx
            if self.is_movable(y,x,dy,dx) and (ny,nx) not in uncrop_pos_set and self.grid_status[ny][nx]["flg"] == False:
                greedy_cultivate(ny,nx,dy,dx)

            for dy2,dx2 in MOVE:
                if [dy2,dx2] == [dy,dx]:
                    continue
                ny,nx = y+dy2,x+dx2
                if self.is_movable(y,x,dy2,dx2) and (ny,nx) not in uncrop_pos_set and self.grid_status[ny][nx]["flg"] == False:
                    greedy_cultivate(ny,nx,dy2,dx2)



        # DFSで、壁にぶち当たるまでは進める方向に突き進み栽培する
        # 栽培する作物は同じ収穫時期で統一する
        ...
        # 遠い座標から順に入り口までの通路の両脇にDFSで収穫時期が同じ作物をひたすら植えていく
        # 途中でもし既に植え終わった地点にぶつかったら、crops_orderから除外、self.cropsへの再追加、grid_statusの更新して
        # そこの栽培はなかったことにする
        uncrop_pos_set = set()
        for dist,base_y,base_x in sorted_cultivatable_pos:
            # print(self.harvest_month,base_y,base_x,num_per_D[self.harvest_month],file=sys.stderr)
            rev_path = self.calc_reversed_path(base_y,base_x)
            uncrop_pos_set |= set(rev_path)
            # 既に植えられている座標がスタート地点だった場合はスキップ
            if self.grid_status[rev_path[0][0]][rev_path[0][1]]["flg"] == True:
                continue

            # ゴールから入り口までのパスを戻る
            for gy,gx in rev_path:
                for dy,dx in MOVE:
                    ny,nx = gy+dy,gx+dx
                    if self.is_movable(gy,gx,dy,dx) and (ny,nx) not in uncrop_pos_set and self.grid_status[ny][nx]["flg"] == False:
                        greedy_cultivate(ny,nx,dy,dx)

                # 既に最短経路上に植えられていた場合はやっぱり植えるのをやめて引っこ抜く
                if self.grid_status[gy][gx]["flg"] == True:
                    crops_order.remove((self.grid_status[gy][gx]["idx"],gy,gx,month))
                    num_per_D[self.grid_status[gy][gx]["D"]] += 1
                    self.grid_status[gy][gx] = {"flg":False, "idx":-1, "S":-1, "D":-1}
                    # TODO:本当はself.cropsに戻す必要があるけどやらなくてもいい結果が出るか先に試す
    ...
    def create_cultivate_plan(self):
        ...
        for month in range(1,self.T):
            ...
            if month == 1:
                self.ini_cultivate_plan(sorted_cultivatable_pos,month,crops_order)
            else:

...
MIN_SD_SPAN = 6 # D-Sがこの値以下の作物はうまみが少ないとしてスキップ
...
class Solver:
    ...
    def calc_move_dist(self):            
        ...
        cultivatable_pos = [] # 栽培可能な座標と、その座標の入り口からの距離
        while queue:
            y,x = queue.popleft()
            cultivatable_pos.append((dists[y][x],y,x))
            for dy,dx in MOVE:
                ny,nx = y+dy,x+dx
    ...
    def create_cultivate_plan(self):
        ...
        cultivatable_pos = self.cultivatable_pos_sort(cultivatable_pos, dists)

        crops_former = deque(self.crops[:self.K//2])
        crops_latter = deque(self.crops[self.K//2:])
        ...
        # 収穫時期がT=50以降の作物を400個選んで並び替えることで埋め尽くす挑戦
        ini_crops = []
        for i,s,d in crops_latter:
        ...
        for dist,y,x in cultivatable_pos:
            ...
            for i in range(len(ini_crops)):
                ...
                crops_latter.remove((crop_idx, s, d))
        ...
        while queue:
            ...
            # そのmonthに栽培開始可能　かつ　植える価値の高い作物を植える
            # TODO:つまり、D-Sが小さい(栽培開始時期が遅い)作物は後回しにする。
            for i in range(len(crops_former)):
                crop_idx, s, d = crops_former[i]
                ...
                self.grid_status[y][x] = {"flg":True, "idx":crop_idx, "S":s, "D":d}
                crops_order.append((crop_idx, y, x, month))
                cultivatable_pos.remove((dists[y][x], y, x))
                max_month_BFS = max(max_month_BFS,d)
                if i == 0:
                    crops_former.popleft()
                else:
                    crops_former.remove((crop_idx, s, d))
            ...
            for dy,dx in MOVE:
                ny,nx = y+dy,x+dx
                if self.is_movable(y,x,dy,dx) and self.grid_status[ny][nx]["flg"] == False:
                    self.grid_status[ny][nx]["flg"] = True
                    queue.append([ny, nx])

        # 2ヶ月目以降の処理を実施する
        for month in range(2,self.T):
            # print(month, file=sys.stderr)
            # print(len(self.crops), file=sys.stderr)
            ...
            ini_crops = []
            for i,s,d in crops_latter:
                if len(ini_crops) == len(cultivatable_pos):
                    break
                if d - s <= MIN_SD_SPAN:
                    continue
                ini_crops.append((i,s,d))
            ...
            for dist, y, x in cultivatable_pos:
                ...
                for i in range(min(600,len(ini_crops))):
            ...
            # 栽培開始時期を過ぎた作物は候補から除外
            while crops_former and crops_former[0][1] <= month:
                crops_former.popleft()

            while crops_latter and crops_latter[0][1] <= month:
                crops_latter.popleft()

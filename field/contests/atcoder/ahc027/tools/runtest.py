...
# for i in li_gt750:
    # 実際のコード実行
    start = time.time()
    cmd = "pypy3 ../a.py < in/{0}.txt > out/{0}.txt".format(str(i).zfill(4))
    returncode = subprocess.run(cmd, shell=True, capture_output=True,text=True)
    end = time.time()

    # ローカルビジュアライズ＆スコア算出
    cmd = "cargo run --release --bin vis in/{0}.txt out/{0}.txt".format(str(i).zfill(4))
    returncode = subprocess.run(cmd, shell=True, capture_output=True,text=True)
    print("{0}.txt".format(str(i).zfill(4)))
    print(returncode.stdout, end="")

    # 標準エラー出力を改行文字で分割して、各行をリストに格納
    stderr_lines = returncode.stdout.split('\n')

...
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/ahc027/aPdjCUIZ_windows.zip).
...
- `-v` The following information for each input is output to standard output in csv format: N, wall(number of 1's in h and v), var_d(variance of d).
...
The color of each square represents the average of dirtiness in a gradation from low:blue to high:red.
It also outputs the score to standard output.

You can also use a [web visualizer](https://img.atcoder.jp/ahc027/aPdjCUIZ.html?lang=en) which is more rich in features.
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc027/aPdjCUIZ_windows.zip)も用意してあります。
...
- `-v` 各入力についての以下の情報をcsv形式で標準出力に出力します: N, wall(hとvにおける1の数), var_d(dの分散)
...
各マスの色は汚れの平均値を低:青→高:赤のグラデーションで表しています。
標準出力にはスコアを出力します。

より機能が豊富な[ウェブ版のビジュアライザ](https://img.atcoder.jp/ahc027/aPdjCUIZ.html?lang=ja)も利用可能です。
